...
# Sectigo CA
CA_SECTIGO="https://acme.sectigo.com/v2/InCommonRSAOV"
CA_SECTIGO_ECC="https://acme.sectigo.com/v2/InCommonECCOV"
...
SSL.com,sslcom
Google.com,google
Google.com_test,google_test,googletest
Sectigo.com,sectigo
"
...
_post() {
  ...
  if [ "$_ACME_CURL" ] && [ "${ACME_USE_WGET:-0}" = "0" ]; then
    ...
    _ret="$?"
    if [ "$_ret" != "0" ]; then
      _err "Please refer to https://curl.se/libcurl/c/libcurl-errors.html for error code: $_ret"
      if [ "$DEBUG" ] && [ "$DEBUG" -ge "2" ]; then
...
_get() {
  ...
  if [ "$_ACME_CURL" ] && [ "${ACME_USE_WGET:-0}" = "0" ]; then
    ...
    ret=$?
    if [ "$ret" != "0" ]; then
      _err "Please refer to https://curl.se/libcurl/c/libcurl-errors.html for error code: $ret"
      if [ "$DEBUG" ] && [ "$DEBUG" -ge "2" ]; then
        _err "Here is the curl dump log:"
        _err "$(cat "$_CURL_DUMP")"
      fi
...
_ns_lookup_cf() {
  _cf_ld="$1"
  _cf_ld_type="$2"
  _cf_ep="https://1.1.1.1/dns-query"
...
_ns_is_available_cf() {
  if _get "https://1.1.1.1" "" 1 >/dev/null 2>&1; then
    return 0
  else
    return 1
  fi
