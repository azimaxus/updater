...
# URLs of GitHub - Updated to Hysteria 2
...
# /etc/hysteria/config.json - Updated for Hysteria 2
...
perform_install() {
  ...
  if [[ -n "$_is_frash_install" ]]; then
    ...
    echo -e "What's next?"
    echo
    echo -e "\t+ Check out the latest quick start guide at $(tblue)https://v2.hysteria.network/docs/getting-started/$(treset)"
    ...
    echo
    echo -e "Check out the latest changelog $(tblue)https://github.com/apernet/hysteria/releases$(treset)"
    echo
  fi
}
